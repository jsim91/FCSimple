                                  ...
                                  add_timestamp = TRUE)
{
  ...
  if(!'collection_instrument' %in% names(fcs_join_obj)) {
    stop("'fcs_join_obj[['collection_instrument']] not found; do fcs_audit() on your object and specify what instrument_type the data was collected with first, either 'cytof' for mass cytometry or 'flow' for fluorescence cytometry")
  }
