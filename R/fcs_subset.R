fcs_subset <- function(fcs_join_obj,
                       ...
                       subset_values)
{
  # fcs_join_obj <- list(data = matrix(data = sample(1:1000,size=10000,replace=TRUE), ncol = 10),
  ...
  if(subset_by=="source") {
    ...
    # only need to subset on data and source entries then return since the other entries depend on these and should not be kept
    fcs_join_obj[["data"]] <- fcs_join_obj[["data"]][get_index,]
    fcs_join_obj[["source"]] <- fcs_join_obj[["source"]][get_index]
    if("run_date" %in% names(fcs_join_obj)) {
      fcs_join_obj[["run_date"]] <- fcs_join_obj[["run_date"]][get_index]
    }
