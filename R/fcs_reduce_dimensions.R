                                  ...
                                  language = c("R","Python"))
{
  ...
  if(tolower(algorithm)=="umap") {
    if(tolower(language)=="r") {
      ...
      system(command = paste0("python ",paste0(capture_dir,"/python/run_umap.py")," ",paste0(capture_dir,"/python/__python_umap_input__.csv")," ",capture_dir))
    ...
    if(tolower(language)=="r") {
      ...
      stop("error in 'language' + 'algorithm': tsne in python not supported (yet)")
    } else {
      stop("error in argument 'language': use 'R' or 'Python'")
    }
  }
