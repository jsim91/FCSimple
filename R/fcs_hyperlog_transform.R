fcs_as.hyperlog <- function(fcs_join_obj, hyperlog_transform_T = 100000,
  ...
  myfs <- split(x = as.data.frame(fcs_join_obj$raw_data), f = fcs_join_obj$source)

  transf_hyperlog <- function(fset, hyper_t, hyper_m, hyper_w, hyper_a) {
    for(i in 1:length(fset)) {
      exprs_data <- fset[[i]]; sample_data <- exprs_data
      for(j in 1:ncol(exprs_data)) {
        ...
        sample_data[,j] <- eval(transform_fun)(exprs_data)
      }
      if(i==1) {
  ...
  tmp_data <- transf_hyperlog(fset = myfs, hyper_t = hyperlog_transform_T, hyper_m = hyperlog_transform_M,
