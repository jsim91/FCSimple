                     ...
                     use_MEM = TRUE, max_alloc = 200000, plot_intensities = FALSE)
{
  ...
  if (!require(gridExtra, quietly = TRUE)) stop("Package 'gridExtra' is required but could not be loaded.")
  ...
  if (!require(RColorBrewer, quietly = TRUE)) stop("Package 'RColorBrewer' is required but could not be loaded.")
  if (!require(cytoMEM, quietly = TRUE)) stop("Package 'cytoMEM' is required but could not be loaded.")
  ...
  clustered_data$cluster[grep("_0$",clustered_data$cluster)] <- "ns"
  ...
  if(length(uclus)>1) {
    ...
    if(use_MEM) {
      ...
      mcalc <- cytoMEM::MEM(exp_data = mem_input, transform=FALSE, choose.markers=FALSE,
