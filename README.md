...
The dimension reduction and clustering steps do offer methods to run some of the calculations in Python, however no knowledge of Python is required. These functions will call Python in the background and results will be transferred to the R environment. The reticulate package is not required. Users may edit the included python scripts if they want to alter the default Python behavior. To take davantage of Python-supported methods, these Python dependencies are required:
