...
{
  if(all('object_history' %in% names(fcs_join_obj), "collection_instrument" %in% names(fcs_join_obj), 'metadata' %in%  names(fcs_join_obj))) { # check if up to date first
    ...
    if(any(length(instrument_type)!=1, !instrument_type %in% c("cytof","flow"))) {
      ...
      fcs_join_obj[['metadata']] <- data.frame(source = fcs_join_obj[['source']])
